...
export const categoriesRepository = {
  findAll: () => prisma.serviceCategory.findMany({ where: { isActive: true }, include: { services: { where: { isActive: true }, orderBy: { name: 'asc' } } }, orderBy: { name: 'asc' } }),
  createWithService: (payload) => prisma.$transaction(async (tx) => {
    ...
    const service = await tx.service.create({ data: { name: payload.serviceName, currentPrice: payload.price, categoryId: category.id } });
